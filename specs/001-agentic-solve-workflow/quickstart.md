...
5. Add `.solve/plans/` and `.solve/reports/` as runtime artifact locations for non-trivial runs.
...
4. Verify that the workflow performs task intake before editing, creates a lightweight plan artifact for non-trivial work, and keeps operator interaction optional by default.
5. Confirm the workflow invokes the isolated review stage before final completion and records validation outcomes.
6. Inspect `.solve/plans/` and `.solve/reports/` for the generated run artifacts.
...
3. Reject any workflow asset that contains task-specific implementation steps, hard-coded source edits, or example assertions tied only to the published task.
...
- The final report lists changed files, validation evidence, review findings, and any blockers.